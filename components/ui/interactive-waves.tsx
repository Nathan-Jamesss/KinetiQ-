...
class Noise {
    ...
    perlin2(x: number, y: number) {
        ...
        x -= Math.floor(x);
        y -= Math.floor(y);
        let fade = (t: number) => t * t * t * (t * (t * 6 - 15) + 10);
        let u = fade(x);
        let v = fade(y);
        let p = this.p;
        ...
        let lerp = (a: number, b: number, x: number) => a + x * (b - a);
...
const animationConfig = {
    ...
    WAVE_TIME_X_FACTOR: 0.008,
    WAVE_NOISE_X_FACTOR: 0.002,
    WAVE_TIME_Y_FACTOR: 0.004,
...
const Waves = () => {
    ...
    const animationState = useRef({
        ...
        lineColor: 'rgba(232, 228, 219, 0.8)', // the beige border color for calming subtle lines
    });
    
    const moved = useCallback((point: any, withCursorForce = true) => {
        const coords = {
    ...
    return (
        <div 
            ref={containerRef} 
            className="waves-container pointer-events-none"
        >
